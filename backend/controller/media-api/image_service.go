...
func UploadImage(ctx *gin.Context) {
	...
	if errUp != nil {
		ctx.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": "Unable To save File => " + errUp.Error()})
		return
	}
...
func ReadAllObjectImage(ctx *gin.Context) {
	var (
	...
	for _, val := range result.Contents {
		if utils.CekExtensionObjectImage(*val.Key) {
			obj.Name = utils.GetNameFileObject(*val.Key)
			obj.Key = *val.Key
			url, _ := service.GetPresignerUrl(sess, bucket, val.Key)
			obj.Url = url
...
func DeleteObjectImage(ctx *gin.Context) {
	var (
		bucket *string
		object *string
	)
